...
describe("filter", function () {
  ...
  it("should call filter impl with correct this", async function () {
    const spy = vi.fn(function* (valStr, diff): Generator<string> {
      // @ts-ignore
  ...
  it("should render a simple filter", async function () {
    expect(await toPromise(new Filter("upcase", (x: string) => x.toUpperCase(), [], liquid).render("foo", ctx))).toBe(
      "FOO",
    );
  });
  ...
  it("should render filters with multiple arguments", async function () {
    ...
    expect(
      await toPromise(
        new Filter("add", (a: number, b: number, c: number) => a + b + c, [two, c], liquid).render(3, ctx),
      ),
    ).toBe("5c");
  });
  ...
  it("should pass Objects/Drops as it is", async function () {
    class Foo {}
    expect(await toPromise(new Filter("name", (a: any) => a.constructor.name, [], liquid).render(new Foo(), ctx))).toBe(
      "Foo",
    );
  });
  ...
  it("should support key value pairs", async function () {
    ...
    expect(
      await toPromise(
        new Filter("add", (a: number, b: number[]) => b[0] + ":" + (a + b[1]), [["num", two]], liquid).render(3, ctx),
      ),
    ).toBe("num:5");
  });
});
