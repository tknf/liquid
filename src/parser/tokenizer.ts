import type { ValueToken, Token, TopLevelToken } from "../tokens";
import {
  FilteredValueToken,
  TagToken,
  HTMLToken,
...
import type { OperatorHandler } from "../render/operator";
import type { TrieNode, Trie } from "../util/operator-trie";
import { createTrie } from "../util/operator-trie";
import type { Operators } from "../render";
...
import { BLANK, NUMBER, QUOTE, SIGN, TYPES, isWord } from "../util/character";
...
export class Tokenizer {
  ...
  *readExpressionTokens(): IterableIterator<Token> {
  ...
  matchTrie<T>(trie: Trie<T>) {
    ...
    let i = this.p;
    let info;
    while (node[this.input[i] as keyof typeof node] && i < this.N) {
      node = node[this.input[i++] as keyof typeof node] as TrieNode<T>;
