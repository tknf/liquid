...
describe("tags/layout", function () {
  ...
  it("should support `options.layouts`", async () => {
    ...
    const liquid = new Liquid({ layouts: "/layouts", fs: nodeFs });
    const html = await liquid.parseAndRender(src);
    return expect(html).toBe("XAY");
  });
  it("should use `layouts` if specified", async function () {
    ...
    const staticLiquid = new Liquid({ root: "/root", layouts: "/layouts", dynamicPartials: false, fs: nodeFs });
  ...
  it("should support relative reference", async function () {
    ...
    const staticLiquid = new Liquid({ root: "/", dynamicPartials: false, fs: nodeFs });
  ...
  it("should support relative root", async function () {
    ...
    const staticLiquid = new Liquid({ root: "./foo", dynamicPartials: false, fs: nodeFs });
